...
// Lazy accessor to break circular dependency: offline/index → offline.sync → auth/offline-strategy → offline/index
// import type se borra en tiempo de compilación → no afecta el ciclo
import type { authSyncHandler, authConflictHandler } from '@features/auth/offline-strategy';

let _authSyncHandler: typeof authSyncHandler | null = null;
let _authConflictHandler: typeof authConflictHandler | null = null;

/* eslint-disable @typescript-eslint/no-require-imports */
function getAuthSyncHandler(): typeof authSyncHandler {
  if (!_authSyncHandler) {
    _authSyncHandler = (
      require('@features/auth/offline-strategy') as typeof import('@features/auth/offline-strategy')
    ).authSyncHandler;
  }
  return _authSyncHandler;
}

function getAuthConflictHandler(): typeof authConflictHandler {
  if (!_authConflictHandler) {
    _authConflictHandler = (
      require('@features/auth/offline-strategy') as typeof import('@features/auth/offline-strategy')
    ).authConflictHandler;
  }
  return _authConflictHandler;
}
...
class OfflineSyncOrchestrator {
  ...
  private handleTaskSuccess(task: OutboxTask, data: any) {
    if (task.url === '/auth/register-guest') {
      getAuthSyncHandler().reconcile(data.usuario);
    }
  }
  ...
  private async handleTaskFailure(task: OutboxTask) {
    if (task.retries < MAX_RETRIES) {
      ...
      if (task.url === '/auth/register-guest') {
        getAuthConflictHandler().handleRegistrationFailure();
      }
      await offlineQueue.removeTask(task.id);
    }
  }
